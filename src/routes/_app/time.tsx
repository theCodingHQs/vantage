...
import { formatCurrency, formatDate, durationToHours, formatDateYYYYMMDD } from '#/lib/utils'
import { Modal } from '#/components/ui/modal'
import { Play, Square, Download, Clock, Plus, Sparkles, HelpCircle } from 'lucide-react'
...
function TimeTrackerPage() {
  ...
  // Zustand timer store
  const { isRunning, seconds, projectId, projectName, startedAt, startTimer, stopTimer, tick } = useTimerStore()
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Description', 'Project', 'Date', 'Duration (Minutes)', 'Hours', 'Billable', 'Hourly Rate', 'Estimated Total']
    ...
    link.setAttribute('download', `vantage_time_export_${new Date().toISOString().split('T')[0]}.csv`)
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-text-2">Measure active billable hours, record timesheets, and export CSV logs.</p>
        </div>
        <div className="flex items-center gap-2">
          <Button size="sm" variant="secondary" onClick={() => setIsManualLogOpen(true)} className="flex items-center gap-1.5">
            <Plus size={14} /> Log Hours Manually
          </Button>
          <Button size="sm" onClick={handleExportCSV} disabled={timeEntries.length === 0} className="flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 bg-gradient-to-br from-surface to-surface-2/40 border-accent/20 relative overflow-hidden">
          ...
          <CardHeader className="pb-2 border-0">
            <div className="flex items-center gap-2 text-xs font-bold text-accent uppercase tracking-wider">
              <span className={`w-2 h-2 rounded-full bg-accent ${isRunning ? 'animate-ping' : ''}`} />
          ...
          <CardContent className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div className="flex-1 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Select
                    ...
                    options={projectsList.map(p => ({ value: p.project.id, label: p.project.title }))}
              ...
              <div className="text-center md:text-right">
                <span className="font-mono text-4xl font-extrabold text-text-1 block tracking-wider">{timeString}</span>
                <span className="text-[10px] text-text-3 font-semibold uppercase block mt-1">elapsed time</span>
              </div>
            </div>
        ...
        <Card className="flex flex-col justify-between">
          <CardHeader className="pb-2 border-0">
            <CardTitle className="text-xs font-bold text-text-2 uppercase">Timesheet Aggregates</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex justify-between items-center py-2 border-b border-border/40">
              ...
              <span className="text-sm font-bold text-accent">
                {totalMin > 0 ? Math.round((billableMin / totalMin) * 100) : 0}% ({durationToHours(billableMin)})
              </span>
            </div>
            <div className="flex justify-between items-center py-2 first:pt-0 last:border-b-0">
              ...
              <span className="text-base font-extrabold text-success">{formatCurrency(totalFinancialVal)}</span>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader className="border-0 pb-2 flex flex-row justify-between items-center">
          <CardTitle className="text-xs font-bold text-text-2 uppercase">Time Logs History</CardTitle>
          <Clock size={16} className="text-text-3" />
        </CardHeader>
        <CardContent>
          <Table
            headers={['Description', 'Project', 'Date', 'Duration', 'Billable', 'Hourly Rate', 'Effective Value']}
            ...
            {timeEntries.map(({ entry, projectName }) => {
              ...
              return (
                <TableRow key={entry.id}>
                  <TableCell className="font-semibold text-text-1">{entry.description || 'General support'}</TableCell>
                  <TableCell>{projectName}</TableCell>
                  <TableCell>{formatDateYYYYMMDD(entry.startedAt)}</TableCell>
                  <TableCell className="font-mono text-accent">{durationToHours(entry.durationMinutes)}</TableCell>
                  <TableCell>
      ...
      <Modal isOpen={isManualLogOpen} onClose={() => setIsManualLogOpen(false)} title="Log Hours Manually" type="right">
        {projectsList.length === 0 ? (
          <div className="py-8 text-center text-text-3 text-xs">Create a project before logging hours.</div>
        ) : (
          <TimeForm
