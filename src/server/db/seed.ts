...
import { 
  users, clients, contacts, deals, projects, tasks, timeEntries, 
  invoices, invoiceItems, expenses, contentCalendar, proposals, 
  clientPortals, portalMessages, files, notifications
} from './schema'
...
async function seed() {
  ...
  const userPortals = await db.select({ id: clientPortals.id }).from(clientPortals).where(eq(clientPortals.userId, userId))
  ...
  const clientNames = [
    ...
    'Hooli', 'Vehement Capital', 'Massive Dynamic', 'Aperture Science', 'Stark Industries',
    'Wayne Enterprises', 'Oscorp', 'Tyrell Corp', 'Cyberdyne Systems', 'Dunder Mifflin',
    'Sterling Cooper', 'E Corp', 'Pied Piper', 'Monarch Sciences', 'Vandelay Industries',
    'Reynholm Industries', 'Bluth Company', 'Prestige Worldwide', 'Gekko & Co', 'Duke & Duke',
    'Nakatomi Plaza', 'Kravitz Corp', 'Sutton Group', 'Summit Partners', 'Bain & Co',
    'Alpha Partners', 'Nova Ventures', 'Zenith Agency', 'Meridian Labs', 'Vanguard Legal',
    'Horizon Logistics', 'Epic Design Studio', 'Pixel Perfect', 'Code Crafters', 'Launchpad Inc'
  ]
  const industries = ['Tech', 'Finance', 'Health', 'Retail', 'Legal', 'Logistics', 'Marketing', 'Consulting']
  ...
  const contactFirstNames = ['Alice', 'Bob', 'Charlie', 'Diana', 'Ethan', 'Fiona', 'George', 'Hannah', 'Ian', 'Julia']
  const contactLastNames = ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Miller', 'Davis', 'Garcia', 'Rodriguez', 'Wilson']
  const roles = ['CEO', 'CTO', 'Product Manager', 'Procurement Lead', 'Director of Engineering', 'VP of Marketing']
  ...
  const dealStages: ('lead' | 'proposal' | 'negotiation' | 'won' | 'lost')[] = ['lead', 'proposal', 'negotiation', 'won', 'lost']
  const dealRows: any[] = []
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const [d] = await db.insert(deals).values({
      ...
      currency: 'USD',
      status,
      probability: status === 'won' ? 100 : status === 'lost' ? 0 : status === 'negotiation' ? 80 : status === 'proposal' ? 55 : 20,
  ...
  const projectStatuses: ('planning' | 'active' | 'on_hold' | 'completed' | 'cancelled')[] = ['planning', 'active', 'on_hold', 'completed', 'cancelled']
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const [p] = await db.insert(projects).values({
      ...
      completedAt: status === 'completed' ? new Date(Date.now() - (5 - (i % 5)) * 24 * 60 * 60 * 1000) : null,
      isBillable: true,
  ...
  const taskTitles = [
    'Research user personas', 'Create design system', 'Draft database schema',
    'Set up Vite project', 'Configure Drizzle ORM', 'Establish server functions',
    'Build authentication screens', 'Build onboarding steps', 'Integrate state manager',
    'Style dashboard metrics', 'Create kanban columns', 'Implement drag-n-drop',
    'Create manual time logging', 'Establish CSV export', 'Configure tailwind theme',
    'Write unit tests', 'Setup client portal routes', 'Integrate Resend emailing',
    'Build social content calendar', 'Create project reports page', 'Deploy client staging',
    'Add PDF export templates', 'Refactor auth middlewares', 'Setup Redis cache fallbacks',
    'Perform SEO audit', 'Fix hydration bugs', 'Optimize image loaders',
    'Draft pricing models', 'Verify responsive forms', 'Setup docker container',
    'Implement chat socket', 'Code split routing tree', 'Optimize SQL indices',
    'Review terms of service', 'Establish stripe billing', 'Refactor store actions',
    'Cleanup mock assets', 'Perform load testing', 'Setup production SSL',
  ...
  for (let i = 0; i < 40; i++) {
    ...
    if (i % 2 === 0 && task.status === 'done') {
      ...
      await db.update(tasks)
        .set({ actualHours: hours.toString() })
  ...
  const invoiceStatuses: ('draft' | 'sent' | 'viewed' | 'paid' | 'overdue' | 'cancelled')[] = ['draft', 'sent', 'viewed', 'paid', 'overdue', 'cancelled']
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const [inv] = await db.insert(invoices).values({
      ...
      dueDate: new Date(Date.now() - (15 - i) * 24 * 60 * 60 * 1000),
      paidAt: status === 'paid' ? new Date(Date.now() - (10 - (i % 5)) * 24 * 60 * 60 * 1000) : null,
  ...
  const expenseDescs = [
    'Figma subscription', 'AWS servers billing', 'Uber for client meeting',
    'M2 Macbook Pro machine', 'Facebook Ads campaign', 'Contractor react-design assistance',
    'Coffee with prospective client', 'Co-working hot desk monthly passes', 'Resend transactional email pro plan',
    'Notion organization software', 'Sentry error logger dashboard subscription'
  ]
  ...
  const platforms: ('twitter' | 'linkedin' | 'instagram' | 'youtube' | 'blog' | 'other')[] = ['twitter', 'linkedin', 'instagram', 'youtube', 'blog', 'other']
  const contentStatuses: ('idea' | 'drafting' | 'scheduled' | 'published')[] = ['idea', 'drafting', 'scheduled', 'published']
  ...
  const proposalStatuses: ('draft' | 'sent' | 'viewed' | 'accepted' | 'declined')[] = ['draft', 'sent', 'viewed', 'accepted', 'declined']
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const [prop] = await db.insert(proposals).values({
      userId,
      clientId: client.id,
      ...
      contentJson: JSON.stringify({
        blocks: [
          ...
          { type: 'paragraph', text: 'This proposal details the design system, codebase migrations, and deployment architecture details.' }
        ]
      }),
      ...
      respondedAt: (status === 'accepted' || status === 'declined') ? new Date(Date.now() - 2 * 24 * 60 * 60 * 1000) : null,
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const [port] = await db.insert(clientPortals).values({
      userId,
      clientId: client.id,
      slug,
      isActive: i % 8 !== 0,
  ...
  const fileNames = [
    'brand_logo_guidelines.pdf', 'neon_database_schema_v2.png', 'figma_workspace_mockup.fig',
    'contractor_outlay_report.xlsx', 'marketing_milestones.pdf', 'website_copywriting_draft.docx',
    'tax_statement_2025.pdf', 'vantage_architecture.png', 'checkout_conversions.xlsx',
    'terms_and_conditions.docx'
  ]
  const mimeTypes = ['application/pdf', 'image/png', 'application/octet-stream', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document']
  ...
  const notifTitles = [
    'Invoice Paid', 'Proposal Signed', 'New Task Assigned', 'Client Sent Message',
    'Contract Milestone Completed', 'Project Nearing Budget Cap', 'Time Log Approved'
  ]
  const notifBodies = [
