...
export const getDashboardData = createServerFn({ method: 'GET' })
  .handler(async () => {
    ...
    const utilizationRate = totalMinutes > 0
    ...
    const activityFeed: { id: string; type: string; title: string; description: string; date: Date }[] = []
...
export const getReportsData = createServerFn({ method: 'GET' })
  .handler(async () => {
    ...
    const projectProfitability = await Promise.all(projectsList.map(async (p) => {
      // time value
      let timeValue = 0
      let totalMinutes = 0
