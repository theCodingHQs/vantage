...
export const Modal: React.FC<ModalProps> = ({
  ...
  if (type === 'center') {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        {/* Container */}
        <div className={`relative w-full bg-surface border border-border rounded-xl shadow-2xl overflow-hidden transition-all transform animate-in zoom-in-95 duration-200 ${sizeWidths[size]}`}>
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-surface-2/40">
            ...
            <button onClick={onClose} className="p-1 text-text-2 hover:text-text-1 hover:bg-surface-2 rounded-lg transition-all">
              <X size={18} />
            </button>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-bg/80 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose} />
