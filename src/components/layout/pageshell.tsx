...
export const PageShell: React.FC<PageShellProps> = ({ user, action, children }) => {
  ...
  return (
    <div className="min-h-screen bg-bg text-text-1 flex">
      ...
      {/* Main Layout Area */}
      <div className=" transition-all flex-1 duration-300 min-h-screen flex flex-col">
        {/* Sticky Header with auto breadcrumbs */}
        <Header breadcrumbs={breadcrumbs} action={action} />

        {/* Core Route Content */}
        <main className="flex-1 px-8 py-4 w-full mx-auto">{children}</main>
      </div>
    </div>
  )
}
